...
export const addThumbnail = TryCatch(async (req: AuthRequest, res) => {
  ...
  const song = await sql`SELECT id FROM songs WHERE id = ${Number(
    req.params.id
  )}`;
...
export const deleteAlbum = TryCatch(async (req: AuthRequest, res) => {
  ...
  const isAlbum = await sql`SELECT * FROM albums WHERE id = ${Number(
    req.params.id
  )}`;
  if (isAlbum.length === 0) {
    res.status(400).json({
...
export const deleteSong = TryCatch(async (req: AuthRequest, res) => {
  ...
  const isSong = await sql`SELECT * FROM albums WHERE id = ${Number(
    req.params.id
  )}`;
  if (isSong.length === 0) {
    res.status(400).json({
