...
const Player = () => {
  ...
  const {song,fetchSingleSong,selectedSong,isPlaying,setIsPlaying,nextSong,prevSong} = useSongData();
    ...
    <div>
      {
        song && <div className="h-[10%]  bg-black flex justify-between items-center text-white px-4">
          <div className="lg:flex items-center gap-4"> 
              <img src={song?.thumbnail ? song.thumbnail: fallbackImg} className="w-12" alt="" />
              <div className="hidden md:block">
                  <p>{song.title}</p>
                  <p>{song.descriptions?.slice(0,30)}....</p>
              </div>
          </div>
          ...
          <div className="flex flex-col items-center m-auto gap-1">
            {
              song.audio && (
                <audio src={song.audio} ref={audioRef}  autoPlay={isPlaying} ></audio>
              )
            }
            <div className="w-full items-center flex font-thin text-green-400">
                <input type="range" min={"0"} max={"100"}  className="progress-bar w-[120px]  md:w-[300px]"  value={(progress/duration)*100  || 0} 
          ...
          <input type="range" min={"0"} max={"100"}  className="w-16 md:w-32" step={"0.01"} value={volume*100} onChange={volumeChange}   
